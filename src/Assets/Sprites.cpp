...
void Sprites::get_AnimatedIcon(const string& url, const std::function<void(AllFramesResult)>& completion) {
    if (iconCache.contains(url)) {
        std::span<uint8_t> bytes = iconCache.at(url);
        ArrayW<uint8_t> spriteArray = BeatLeader::toArray(bytes);
...
void Sprites::GetCountryIcon(string country, const std::function<void(UnityEngine::Sprite*)>& completion) {
    get_Icon("https://cdn.beatleader.xyz/flags/" + toLower(std::move(country)) + ".png", completion);
}
